...
private:
  ...
  std::chrono::time_point<clock>
      start_time;          ///< Last time the timer was started
...
public:
  ...
  void update(uint32_t work_progress) {
// Provide simple way of optimizing out progress updates
    ...
    uint16_t percent =
        (uint8_t)(work_done * omp_get_num_threads() * 100 / total_work);
    ...
    std::cerr << "\r\033[2K[" << std::string(percent / 2, '=')
              ...
              << timer.lap() / percent * (100 - percent) << "s - "
